...
const AdminDashboard = () => {
  ...
  const handleResetPassword = async (userId, userName) => {
    ...
    try {
      ...
      if (res && res.data && res.data.data && res.data.data.tempPassword) {
        setTempPasswordToShow(res.data.data.tempPassword);
    ...
    <div className="space-y-6 text-left">
              ...
              <table className="w-full text-xs text-left text-slate-600">
                <thead className="text-[10px] font-bold uppercase bg-slate-50 text-slate-500 border-b border-slate-100">
                  <tr>
                    ...
                    <th className="px-4 py-3">Password State</th>
                    ...
                    <tr key={u._id} className="hover:bg-slate-50/50">
                      ...
                      <td className="px-4 py-3">
                        <span className={`px-2 py-0.5 rounded text-[9px] font-bold uppercase ${
                          u.passwordState === 'Active'
                            ? 'bg-emerald-50 text-emerald-600 border border-emerald-100'
                            : u.passwordState === 'Temporary Password'
                      ...
                      <td className="px-4 py-3">
                        ...
                        <button
                          ...
                          disabled={submitting}
                          className="ml-2 px-3 py-1 rounded text-[10px] font-bold bg-slate-50 hover:bg-slate-100 text-slate-650 border border-slate-200 transition-all focus:outline-none"
                        >
                          Reset Password
                        </button>
                      </td>
      ...
      {showTempPasswordModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
          <div className="bg-white border border-slate-100 shadow-2xl rounded-2xl p-8 max-w-md w-full text-center">
            <h3 className="text-xl font-black text-slate-800 tracking-tight">Temporary Password Generated</h3>
            <p className="text-sm font-semibold text-slate-400 mt-2">
              The user's password has been successfully reset. Displaying temporary credential below.
            </p>
            <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 my-6 font-mono text-lg font-bold text-slate-850 select-all cursor-pointer" title="Click to copy">
              {tempPasswordToShow}
            </div>
            <p className="text-xs text-rose-500 font-bold uppercase tracking-wider mb-6">
